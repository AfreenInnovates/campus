...
function Key({ children, light = false, small = false }: { children: ReactNode; light?: boolean; small?: boolean }) {
  const style: CSSProperties = small ? { height: "1.2rem", minWidth: "1.2rem", fontSize: "0.58rem", padding: "0 0.25rem" } : {};
  return (
    <kbd className="inline-grid place-items-center border-2 border-ink bg-paper font-mono font-bold uppercase tracking-wider text-ink shadow-[2px_2px_0_var(--ink)]" style={style}>
      {children}
    </kbd>
  );
}
...
function Bar({ label, value, color, danger }: { label: string; value: number; color: string; danger?: boolean }) {
  ...
  return (
    <div className="w-40">
      <div className="mb-1 flex items-center justify-between text-[10px] font-black uppercase tracking-[0.16em] text-ink-soft">
        <span>{label}</span>
      ...
      <div className="h-2.5 w-full border-2 border-ink bg-paper">
        <div className="h-full transition-[width] duration-150 border-r-2 border-ink" style={{ width: `${Math.max(0, Math.min(100, value))}%`, background: fill }} />
      </div>
    </div>
  );
}
...
function LocationHeader({ tag }: { tag: string }) {
  ...
  return (
    <div className="pointer-events-auto flex items-center gap-3 border-2 border-ink bg-paper px-4 py-2 shadow-[4px_4px_0_var(--ink)]">
      <div className="flex flex-col">
        <div className="text-[10px] font-black uppercase tracking-widest text-ink-soft">{block}</div>
        <div className="text-sm font-black tracking-wide text-ink">{place}</div>
      </div>
    </div>
  );
}
...
function ObjectivesPanel() {
  ...
  return (
    <section className="brutal-panel pointer-events-auto w-[min(19.5rem,calc(100vw-1.5rem))] p-4" aria-label="Objectives">
      <div className="flex items-center justify-between border-b-2 border-ink pb-2">
        <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-ink">Objectives</h2>
            ...
            <li key={id} className={`${current ? "flex" : "hidden sm:flex"} items-start gap-2 text-[12px] leading-snug`}>
              <span
                className={`mt-0.5 grid h-4 w-4 shrink-0 place-items-center border-2 text-[9px] font-black ${
                  complete ? "border-ink bg-mint text-ink" : current ? "border-ink bg-sun text-ink" : "border-ink bg-paper text-transparent"
                }`}
                aria-hidden
              >
                {complete ? "✓" : current ? "›" : ""}
              ...
              <div className="min-w-0">
                <div className={complete ? "text-ink-soft line-through decoration-ink-soft" : current ? "font-black text-ink" : "font-bold text-ink-soft"}>
                  {scenarioObjectById(id).label}
                  <span className="ml-1.5 text-[9px] font-black uppercase tracking-wider text-ink-soft">{placeName(id)}</span>
                </div>
                {current && <p className="mt-1 text-[11px] font-bold leading-snug text-ink">{guidance.instruction}</p>}
              </div>
            </li>
          );
        })}
...
function Vitals() {
  ...
  return (
    <div className="brutal-panel flex flex-col gap-3 p-4">
      <Bar label="Health" value={health} color="var(--mint)" danger={health < 35} />
      <Bar label="Air" value={air} color="#6fb8ff" danger={air < 35} />
      <div className="text-[9px] font-black uppercase tracking-[0.16em] text-ink-soft">
...
function PromptBar() {
  ...
  return (
    <div key={prompt} className="hud-rise flex max-w-[min(34rem,calc(100vw-2rem))] items-center gap-3 border-2 border-ink bg-paper px-4 py-2.5 text-[13px] font-bold text-ink shadow-[4px_4px_0_var(--ink)]">
      {action ? (
        <>
          <span className="text-ink-soft">Press</span> <Key light>E</Key> <span className="text-ink-soft">to</span> {action[1]}
        </>
      ) : (
        <>
          <span className="brutal-tag bg-coral text-ink">Heads up</span>
          {prompt}
        </>
      )}
    </div>
...
function ControlsHint() {
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-3 border-2 border-ink bg-paper px-4 py-3 text-[11px] font-black uppercase tracking-wider text-ink-soft shadow-[4px_4px_0_var(--ink)]">
      {items.map(([key, label]) => (
...
function NarrationCaption() {
  ...
  return (
    <div key={caption.id} className="hud-rise brutal-panel max-w-[min(40rem,calc(100vw-2rem))] p-4 text-center text-sm font-bold leading-relaxed text-ink" role="status" aria-live="polite">
      <span className="brutal-tag bg-sun mr-3">Narrator</span>
      {caption.text}
    </div>
  );
}
...
function EvidencePanel() {
  ...
  return (
    <div className="brutal-panel pointer-events-auto w-[min(22rem,calc(100vw-1.5rem))] p-4">
      <div className="flex items-baseline justify-between border-b-2 border-ink pb-2">
        <span className="text-[10px] font-black uppercase tracking-[0.18em] text-ink">Evidence</span>
        <span className="font-mono text-[10px] text-ink-soft font-bold">{evidence.length} items</span>
      </div>
      {evidence.length === 0 ? (
        <p className="mt-3 text-[11px] font-bold text-ink-soft">Waiting for the sector feed.</p>
      ) : (
        <ul className="mt-2 space-y-2">
          {evidence.map((item) => {
            ...
            return (
              <li key={item.id} className="border-b-2 border-ink pb-3 mb-3 last:border-0 last:pb-0 last:mb-0">
                ...
                <div className="mt-1 text-[11px] font-bold leading-snug text-ink">{item.nextAction}</div>
...
function Log() {
  ...
  return (
    <div className="brutal-panel w-[min(20rem,calc(100vw-1.5rem))] p-4">
      <div className="mb-2 flex items-center justify-between border-b-2 border-ink pb-2 text-[10px] font-black uppercase tracking-[0.18em] text-ink">
        <span>Drill log</span>
        <span className="text-ink-soft">{log.length}/6</span>
      </div>
      <div className="space-y-1.5 text-right">
        {log.map((entry, index) => (
            <div
            key={entry.id}
            className="border-b border-ink/20 pb-2 text-[11px] font-bold leading-snug last:border-0 last:pb-0"
            style={{
              color: entry.tone === "bad" ? "var(--danger)" : entry.tone === "good" ? "var(--mint)" : "var(--ink)",
...
function ConnectionBadge() {
  ...
  return (
    <span className="flex items-center gap-2 border-2 border-ink bg-paper px-3 py-2 font-mono text-[10px] font-black uppercase shadow-[3px_3px_0_var(--ink)]" style={{ color }}>
...
function RouteMessageCard() {
  ...
  return (
    <div className="hud-rise max-w-[min(30rem,calc(100vw-1.5rem))] brutal-panel px-5 py-4" role="status" aria-live="polite">
      <div className="text-[10px] font-black uppercase tracking-[0.18em] text-mint">Message from the warden · {message.confidence}</div>
      <div className="mt-1 text-sm font-black tracking-wide text-ink">{message.caption}</div>
      <div className="mt-1 text-[10px] font-bold text-ink-soft">Disappears in {Math.ceil((message.expiresAt - now) / 1000)}s · you choose the route</div>
    </div>
  );
}
...
function HazardBanner() {
  ...
  return (
    <div className="flex max-w-[min(26rem,calc(100vw-1.5rem))] items-center gap-4 border-2 border-ink bg-paper p-4 shadow-[5px_5px_0_var(--ink)]" style={{ borderLeft: `6px solid ${alert.color}` }} role="status" aria-live="polite">
      <span className="signal-pulse h-3 w-3 shrink-0 rounded-full border-2 border-ink" style={{ background: alert.color }} />
      <div>
        <div className="text-[11px] font-black uppercase tracking-[0.17em]" style={{ color: alert.color }}>
          {alert.label}
        </div>
        <div className="mt-0.5 text-[12px] font-bold text-ink-soft">{alert.detail}</div>
      </div>
    </div>
  );
}
...
function CommandDeck() {
  ...
  return (
    <div className="brutal-panel w-full p-4 sm:w-[min(42rem,calc(100vw-1.5rem))]">
      <div className="flex items-center justify-between gap-3 border-b-2 border-ink pb-2">
        <div className="text-[11px] font-black uppercase tracking-[0.18em] text-ink">Warden commands</div>
        <span className="font-mono text-[10px] font-bold text-ink-soft">sector / {mode.sectorId}</span>
      </div>
        ...
        {COMMANDS.map((command) => {
          ...
          return (
            <button
              ...
              className="min-w-0 flex-1 border border-paper/10 bg-black/40 rounded px-2 py-2.5 text-center transition enabled:hover:border-paper/30 disabled:cursor-not-allowed disabled:opacity-30 sm:min-w-[6.5rem] sm:py-2"
              style={{ borderTopColor: command.color, borderTopWidth: 2 }}
            >
              <span className="block font-mono text-[9px] font-bold" style={{ color: command.color }}>
                {sent === command.code ? "SENT" : command.label}
              </span>
              <span className="mt-1 hidden truncate text-[10px] text-paper/60 sm:block">{command.detail}</span>
            </button>
          );
        })}
      </div>
...
function PhoneCollapsible({ label, children }: { label: string; children: ReactNode }) {
  ...
  return (
    <div className="pointer-events-auto flex flex-col items-end gap-2">
        ...
        aria-expanded={open}
        className="flex items-center gap-2 border-2 border-ink bg-paper px-4 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-ink shadow-[4px_4px_0_var(--ink)]"
      >
        {label}
        <span aria-hidden className="text-violet">{open ? "–" : "+"}</span>
      </button>
      {open && children}
    </div>
  );
