...
function Head() {
  return (
    <group position={[0, 1.72, 0]} scale={0.9}>
...
function Torso() {
  return (
    <>
      <Part color={COLORS.jacket} position={[0, 1.12, 0]} scale={[1.0, 1.1, 0.65]}>
        <capsuleGeometry args={[0.19, 0.35, 6, 14]} />
      </Part>
...
function Arm({
  ...
  return (
    <group position={[0.26 * side, 1.4, 0]} rotation={[0, 0, 0.08 * side]}>
      <group ref={swing}>
        <Part color={COLORS.jacket} position={[0, -0.26, 0]}>
...
function Leg({ side, swing }: { side: 1 | -1; swing: RefObject<THREE.Group | null> }) {
  return (
    <group ref={swing} position={[0.11 * side, 0.85, 0]}>
      <Part color={COLORS.pants} position={[0, -0.4, 0]}>
  ...
  useFrame((_, rawDt) => {
    ...
    if (upper.current) {
      upper.current.position.y =
        Math.abs(Math.sin(state.phase)) * 0.035 * stride + Math.sin(state.time * 2.5) * 0.015 * (1 - stride);
      upper.current.rotation.x = Math.sin(state.time * 2.5) * 0.02 * (1 - stride);
    }
  });
